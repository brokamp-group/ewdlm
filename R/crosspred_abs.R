...
#
crosspred_abs <- function(basis, model=NULL, coef=NULL, vcov=NULL, model.link=NULL, at=NULL,
                          from=NULL, to=NULL, by=NULL, lag, bylag=1, cen=NULL, ci.level=0.95,
                          cumul=FALSE, abs_sum=FALSE) {
  #
  ################################################################################
  ...
  if (abs_sum) {
    absfit <- as.vector(Xpredall %*% abs(coef))
    absse <- sqrt(pmax(0,rowSums((Xpredall%*%vcov)*Xpredall)))
    names(absfit) <- names(absse) <- predvar
  }
  ...
  if(abs_sum) list <- c(list, list(absfit=absfit, absse=absse))
  #
  ...
  if(!is.null(model.link) && model.link %in% c("log","logit")) {
    ...
    if(abs_sum) {
      list$absRRfit <- exp(absfit)
      list$absRRlow <- exp(absfit-z*absse)
      list$absRRhigh <- exp(absfit+z*absse)
    }
  } else {
    ...
    if(abs_sum) {
      list$abslow <- absfit-z*absse
      list$abshigh <- absfit+z*absse
    }
  }
  #
